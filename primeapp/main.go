...
func main() {
	...
	// create a channel to indicate whe the user wants to quit
...
func checkNumbers(scanner *bufio.Scanner) (string, bool) {
	...
	// try to convert what the user type into an input
...
func isPrime(n int) (bool, string) {
	...
	// negative numbers are not isPrime
	if n < 0 {
		return false, "Negative number are not prime, by definition!"
	}

	// use the modulus operator repeatedly to see id we have a prime number
	for i := 2; i <= n/2; i++ {
		if n%i == 0 {
			// not prime number
